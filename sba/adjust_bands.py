...
AVAILABLE_CHANNELS = ['VIS006', 'VIS008', 'NIR016', 'IR_039', 'IR_062', 'IR_073', 
...
class SpectralBandAdjustment:
    def __init__(self, input_sat, input_sat_data, target_sat='METEOSAT11-SEVIRI', verbose=False):
        """ input_sat (str):       Name of input satellite to be transformed. 
                                   Select from AVAILABLE INPUT_SATS 
            input_sat_data (dict): Dictionary of input satellite data for each channel.
                                   Channel name (from AVAILABLE_CHANNELS) should be mapped
                                   to corresponding numpy arrays. {'ch1': array1, 'ch2': array2, ...}
            target_sat (str):      Name of satellite to be mimiced. Until now only METEOSAT11-SEVIRI
                                   is available (MSG4).
        """
    ...
    def transform(self):
        """ Linearily transform input_sat_data to mimic the target satellite's spectral band. """
        if self.verbose:
            print('   ----- TRANSFORMING DATA FROM {} TO MIMIC {} -----\n'.format(self.input_sat, self.target_sat))
        if self.is_transformed:
            print('   >>> WARNING: For transform, the data already have been transformed.')
        ...
        for ch in AVAILABLE_CHANNELS:
            if ch in self.transformed.keys():
                ...
                self.transformed[ch] = self._apply_transform(slope, offset, self.transformed[ch])
                if self.verbose:
                    print('   >>> Transformed {} with slope={:.4f} and offset={:.4f}'.format(ch, slope, offset))
            else:
                if self.verbose:
                    print('   >>> Channel {} not found in input_sat_data => Skipping'.format(ch))
    ...
    def inverse_transform(self):
        """ Linearily re-transform data to mimic input satellite's spectral band. """
        if self.verbose:
            print('   ----- RE-TRANSFORMING DATA FROM {} TO {} -----\n'.format(self.target_sat, self.input_sat))
        if not self.is_transformed:
            print('   >>> WARNING: For inverse_transform, the data have not been transformed yet.')

        for ch in AVAILABLE_CHANNELS:
            if ch in self.transformed.keys():
                slope = COEFFS_MET11_NIGHT[self.input_sat][ch]['slope']
                offset = COEFFS_MET11_NIGHT[self.input_sat][ch]['offset']
                self.transformed[ch] = self._apply_inverse_transform(slope, offset, self.transformed[ch])
                if self.verbose:
                    print('   >>> Inverse transformed {} with slope={:.4f} and offset={:.4f}'.format(ch, slope, offset))
            else:
                if self.verbose:
                    print('   >>> Channel {} not found in input_sat_data => Skipping'.format(ch))
